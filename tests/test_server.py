...
class TestRetryMechanism(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.models_patcher = patch.dict(
            "anthropic_proxy.client.CUSTOM_OPENAI_MODELS", 
            self.mock_models, 
            clear=True
        )
        self.config_patcher = patch(
            "anthropic_proxy.client.config", 
            self.mock_config
        )
    ...
    def test_create_claude_client_with_retry(self, mock_transport_class, mock_client_class):
        ...
        mock_transport_class.assert_called_once_with(retries=ModelDefaults.DEFAULT_MAX_RETRIES)
    ...
    @patch('anthropic_proxy.client.httpx.AsyncClient')
    @patch('anthropic_proxy.client.httpx.AsyncHTTPTransport')
    def test_create_openai_client_with_retry(self, mock_transport_class, mock_http_client_class, mock_openai_class):
    ...
    @patch('anthropic_proxy.client.httpx.AsyncClient')
    @patch('anthropic_proxy.client.httpx.AsyncHTTPTransport')
    def test_claude_client_retry_configuration_details(self, mock_transport_class, mock_client_class):
        ...
        mock_transport_class.assert_called_once_with(retries=2)  # DEFAULT_MAX_RETRIES = 2
    ...
    @patch('anthropic_proxy.client.httpx.AsyncClient')
    @patch('anthropic_proxy.client.httpx.AsyncHTTPTransport')
    def test_openai_client_retry_configuration_details(self, mock_transport_class, mock_http_client_class, mock_openai_class):
    ...
    def test_mock_transport_success_response(self):
        ...
        async def _test():
            ...
            def success_handler(request):
                self.retry_attempts.append(request)
                return httpx.Response(200, json={"success": True, "message": "Mock response"})
...
class TestRetryIntegration(unittest.TestCase):
    ...
    @patch('anthropic_proxy.client.httpx.AsyncClient')
    @patch('anthropic_proxy.client.httpx.AsyncHTTPTransport')
    def test_end_to_end_retry_configuration(self, mock_transport_class, mock_client_class):
        ...
        with patch('anthropic_proxy.client.httpx.AsyncHTTPTransport') as mock_transport:
            with patch('anthropic_proxy.client.httpx.AsyncClient'):
                with patch('anthropic_proxy.client.AsyncOpenAI'):
                    ...
                    mock_transport.assert_called_with(retries=ModelDefaults.DEFAULT_MAX_RETRIES)
    ...
    def test_retry_transport_creation_verification(self):
        ...
        retry_transport = httpx.AsyncHTTPTransport(retries=ModelDefaults.DEFAULT_MAX_RETRIES)
        ...
        async def _test():
            def success_handler(request):
                return httpx.Response(200, json={"success": True, "message": "Request succeeded"})
        ...
        with patch('anthropic_proxy.client.httpx.AsyncHTTPTransport') as mock_transport:
            with patch('anthropic_proxy.client.httpx.AsyncClient') as mock_http_client:
                with patch('anthropic_proxy.client.AsyncOpenAI') as mock_openai:
                    ...
                    # Verify HTTP client was created with transport
                    mock_http_client.assert_called_with(transport=mock_transport.return_value)
