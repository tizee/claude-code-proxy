...
def parse_streaming_response_tokens(response_text: str, is_anthropic_format: bool = False) -> Dict[str, int]:
    ...
    try:
        if is_anthropic_format:
            ...
            usage_pattern = r'"usage":\s*{\s*"input_tokens":\s*(\d+),\s*"output_tokens":\s*(\d+)'
            matches = re.findall(usage_pattern, response_text)
            if matches:
                input_tokens, output_tokens = map(int, matches[-1])  # Take the last usage info
            else:
...
class TokenizedTestResult:
    ...
    def calculate_derived_metrics(self):
        ...
        if self.total_duration > 0:
            ...
            generation_time = (self.total_duration - self.ttfc) / 1000  # Convert to seconds
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "Average Speed (bytes/sec)": (self.total_content_length / (self.total_duration / 1000)) if self.total_duration > 0 else 0,
            "error": self.error
        }


async def run_tokenized_test(name: str, client: httpx.AsyncClient, url: str, payload: Dict[str, Any], headers: Dict[str, str], model_id: str) -> TokenizedTestResult:
    ...
    try:
        ...
        is_anthropic_format = ("v1/messages" in url or "test_conversion" in url)  # Proxy endpoints
        token_info = parse_streaming_response_tokens(result.response_text, is_anthropic_format)
        ...
        if result.total_tokens > 0 and model_id in CUSTOM_OPENAI_MODELS:
            ...
            result.cost = (result.input_tokens * input_cost_per_token) + (result.output_tokens * output_cost_per_token)
...
async def main(args):
    ...
    direct_payload["model"] = model_config["model_name"]  # Use the model_name for direct API
    ...
    direct_headers = {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}
    ...
    async with httpx.AsyncClient() as direct_client:
        direct_results = await run_tokenized_test("DIRECT API", direct_client, direct_api_url, direct_payload, direct_headers, model_id)
    ...
    async with httpx.AsyncClient() as proxy_client:
        proxy_results = await run_tokenized_test("PROXY SERVER", proxy_client, test_conversion_url, proxy_payload, {"Content-Type": "application/json"}, model_id)
    ...
    if not direct_results.error and not proxy_results.error:
        ...
        throughput_loss = ((direct_tps - proxy_tps) / direct_tps * 100) if direct_tps > 0 else 0
        
        # Translation overhead analysis
        response_size_overhead = proxy_results.total_content_length - direct_results.total_content_length
        translation_overhead_percent = (response_size_overhead / direct_results.total_content_length * 100) if direct_results.total_content_length > 0 else 0
        ...
        print(f"      • Proxy (Anthropic): {proxy_results.total_content_length:,} bytes")
        print(f"      • Format Overhead: {response_size_overhead:+,} bytes ({translation_overhead_percent:+.1f}%)")
        
        print(f"\n   🎯 Token Accuracy:")
        print(f"      • Input Tokens: {direct_results.input_tokens} (direct) vs {proxy_results.input_tokens} (proxy) [{token_input_diff:+d}]")
        print(f"      • Output Tokens: {direct_results.output_tokens} (direct) vs {proxy_results.output_tokens} (proxy) [{token_output_diff:+d}]")
        print(f"      • Consistency: {'✅ Perfect Match' if tokens_consistent else '⚠️  Tokens Differ'}")
        ...
        if overhead_ttfc > 100:
            print(f"   • High TTFC overhead ({overhead_ttfc:.0f}ms) - Check network latency or processing delays")
        if throughput_loss > 15:
            print(f"   • Significant throughput loss ({throughput_loss:.1f}%) - Consider request processing optimizations")
        if translation_overhead_percent > 30:
            print(f"   • Large format overhead ({translation_overhead_percent:.1f}%) - Anthropic format is verbose")
        if not tokens_consistent:
            print(f"   • Token inconsistency detected - May indicate translation issues")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Token-based Performance Analysis for Claude Proxy Server")
    parser.add_argument("--model_id", "-m", type=str, required=True, help="The model ID to test (must be in models.yaml)")
